...
*	to remove sass partials and header/template calls from the source.
...
foreach($user_fxns as $k => $v){
	...
	foreach(scandir($base_dir."/.includes/templates") as $file) {
		if(substr($file,0,1) == '.' 
			|| substr($file,0,1) == '0' 
			|| substr($file,0,2) == '..' 
			|| substr($file,0,2) === '/.' 
			|| substr($file,0,2) === '/0') continue;
		$dir = $base_dir."/.includes/templates".DIRECTORY_SEPARATOR.$file;
		if(is_dir($dir)) {
			$temp = str_replace($base_site,"",$dir);
			if($temp[0] != "."){
				$template_dirs []= $temp;
				$template_dirs = array_merge($template_dirs, expandDirectories($dir));
			}
		}
	}
	//print_r ($header_dirs);
	...
	foreach($user_fxns as $p => $a){
		...
		foreach($template_dirs as $k => $v){
			...
			if(preg_match_all($regex,$file,$matches) > 0){
				$string .= "<li>".preg_match_all($regex,$file,$matches) ." times in template ".end($segments)."</li>";
				$count += preg_match_all($regex,$file,$matches);
			}
		}
